...
portfolioApp.controller('historicalController', ['$scope', '$rootScope', '$http', function($scope, $rootScope, $http){
	...
	$http({
		...
		for (var i = 0; i < arrLen; i++){
			chartDataValue.push(Math.round(res.data[i].Total).toPrecision(2));
		...
		for (var key in rawHistoricalArray){
			...
			for (var inKey in rawHistoricalArray[key]) {
				if (!HistoryData[key][inKey]['Value'] == 0) {
					console.log('Value before rounding: ' + HistoryData[key][inKey]['Value']);
					console.log('Value after rounding: ' + parseFloat(Math.round(HistoryData[key][inKey]['Value'] * 100 ) / 100 ));
					dataKeyArray.push(Math.round(HistoryData[key][inKey]['Value'] * 100 ) / 100 );
				}
				if (!HistoryData[key][inKey]['FundName'] == '') {
					HistoricalLabels.push((a = new Date(HistoryData[key][inKey]['Date']), `${a.getDate()}/${a.getMonth()}/${a.getFullYear().toString().substr(-2)}`));
					//HistoricalLabels.push(HistoryData[key][inKey]['Date']);
				}
		...
		ctx_total.canvas.height = chart_height;
		ctx_total.canvas.width = chart_width;
